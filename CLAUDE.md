...
- **`tester.pumpAndSettle()` never converges while a `TextField` is focused.** Its cursor-blink animation keeps a frame scheduled forever, so `pumpAndSettle()` spins until it hits its own internal timeout — it isn't a real app hang, it just looks like one (near-zero CPU, no progress). This bites any test that does `enterText(...)` and then `pumpAndSettle()` for a subsequent async step while that field is still focused. Fix: use a small bounded number of explicit `await tester.pump();` calls instead (or unfocus the field first via `FocusManager.instance.primaryFocus?.unfocus()`). `pumpAndSettle()` is safe again once no `TextField` is focused (e.g. after navigating to a screen with no `autofocus: true` field).
